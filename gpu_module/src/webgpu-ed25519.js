...
@group(0) @binding(4) var<storage, read> prefixData: Prefix;
...
export class WebGpuEd25519Scanner {
  ...
  async initialize() {
    ...
    this.prefixBuffer = this.device.createBuffer({
  ...
  createBindGroup() {
    ...
    this.bindGroup = this.device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 4, resource: { buffer: this.prefixBuffer } }
      ]
    });
  ...
  async scanBatchBitset(scalarWords, prefixBytes, prefixNibbleLength) {
    const runScan = async () => {
      ...
      const params = new Uint32Array([batchSize, prefixNibbleLength, 0, 0]);
      const prefixWords = new Uint32Array(32);
      prefixWords.set(prefixBytes.slice(0, 32));

      this.device.queue.writeBuffer(this.scalarBuffer, 0, scalarWords);
      this.device.queue.writeBuffer(this.paramsBuffer, 0, params);
      this.device.queue.writeBuffer(this.prefixBuffer, 0, prefixWords);
